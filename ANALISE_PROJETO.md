...
### 3. `confirm_order` não valida estoque
**Confirmado ao vivo (2026-08-26)**, lendo a definição completa da função e testando contra produção: a RPC valida `products.active` e `store_id`, mas nunca olha `stock_quantity`/`track_stock`. O client também não reconcilia carrinho com estoque vivo em nenhum momento (item fica congelado no carrinho desde que foi adicionado). Teste real: produto com `track_stock=true`/`stock_quantity=0` corretamente some do cardápio (filtro client-side funciona), mas `confirm_order` aceitou o pedido do mesmo jeito (`200`, pedido criado). Hoje isso não afeta nenhum cliente real — nenhum produto em nenhuma das 2 lojas tem `track_stock=true` (todas ignoram controle de estoque). Fica pendente pra quando existir produto com estoque controlado de verdade — nessa hora, a validação de servidor deveria entrar junto com o item 2 (Realtime de estoque), não antes.
